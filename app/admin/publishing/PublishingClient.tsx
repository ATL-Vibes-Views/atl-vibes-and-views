              ...
              <div key={post.id} className={`bg-white border border-[#e5e5e5] border-l-4 ${borderColor}`}>
                <div className="px-5 py-4">
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    <button
                      onClick={() => post.slug && window.open(`/hub/stories/${post.slug}`, "_blank")}
                      disabled={!post.slug}
                      className={`inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full border transition-colors ${post.slug ? "border-[#e5e5e5] text-[#374151] hover:border-[#d1d5db]" : "border-[#e5e5e5] text-[#9ca3af] cursor-not-allowed"}`}
                    >
                      Preview
                    </button>
                    <button
